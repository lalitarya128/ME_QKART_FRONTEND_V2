import {
   ...
   *
   */
  export const getTotalCartValue = (items = [],shippingCharge = 0) => {
    if (!items.length) {
      return 0;
    }
    const total = items
      .map((items) => items.cost * items.qty)
   ...
   *
   * @returns { Number }
   *    Value of toatl items in the cart
   *
   */
   ...
   *
   */
  const Cart = ({ products, items = [], handleQuantity ,isReadOnly=false,shippingCharge = 0}) => {
      ...
      <>
        <Box className="cart">
          ...
          >
            ...
            >
              ${getTotalCartValue(items,shippingCharge)}
            </Box>
          </Box>
