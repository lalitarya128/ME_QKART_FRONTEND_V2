...
const Checkout = () => {
  ...
  const validateRequest = (items, addresses) => {
    ...
    const cartValue = getTotalCartValue(items,shippingCharge);
    ...
    if(validateRequest(items,addresses)){
      ...
      try {
        ...
        if (e.response) {
          ...
          enqueueSnackbar(
            "Checkout is not proceesed. Check that the address is selected, reachable and returns valid JSON.",
            {
              variant: "error",
            }
          );
  ...
  const createShippingChargeByAddressId = (addressId)=> {
    let min = 1;
    let max = 30;
    setAddresses({ ...addresses, selected: addressId })
    setShippingCharge(Math.floor(Math.random() * (max - min)) + min);
    return true;
  }
      ...
      <Grid container>
        <Grid item xs={12} md={9}>
          <Box className="shipping-container" minHeight="100vh">
            ...
            <Divider />
            <Box sx={{ mt: 2 }}>
              {/* TODO: CRIO_TASK_MODULE_CHECKOUT - Display list of addresses and corresponding "Delete" buttons, if present, of which 1 can be selected  ()=>setAddresses({ ...addresses, selected: add._id })*/}
              {addresses.length !== 0 ? 
                ( 
                  addresses['all'].map((add) =>{
                   return <Grid container direction="row"
                   justifyContent="space-between" sx={{ border: '1px solid',borderColor: 'grey.500',p: 1, py: 3, mb:2,borderRadius: 2,}} className={addresses.selected === add._id ? 'selected': 'not-selected'}  key={add._id}  onClick={()=>createShippingChargeByAddressId(add._id)}>
                    <Grid item>{add['address']} </Grid>
                    <Grid item>
                      <Button color="success" startIcon={<Delete />}  onClick={()=>deleteAddress(token,add._id)}>
                        Delete
                      </Button>
                    </Grid>
                   </Grid>
            ...
            <Box my="1rem">
              ...
              <Typography>
                Pay ${getTotalCartValue(items,shippingCharge)} of available $
            ...
            <Button
              ...
              variant="contained"
              onClick={()=>performCheckout(token, items, addresses,shippingCharge)}
            >
              PLACE ORDER
            </Button>
          </Box>
        ...
        <Grid item xs={12} md={3} bgcolor="#E9F5E1">
          <Cart isReadOnly products={products} items={items} />
           <Box className="cart" p={2}>
            <Typography variant="h4" fontWeight="700" fontSize="1.5rem">Order Details</Typography>
            ...
            <Box display="flex" alignItems="flex-start"justifyContent="space-between" >
              <Typography variant="h5" fontWeight="700" fontSize="1.5rem" >Total</Typography>
              <Typography variant="h5" fontWeight="700" fontSize="1.5rem">${getTotalCartValue(items,shippingCharge)}</Typography>
            </Box>
           </Box>
        </Grid>
